...
export default function useAccounts() {
  ...
  const rebuildAccounts = useCallback((raw, json) => {
    ...
    console.log('[rebuildAccounts]', accountNames.size, 'unique names ->', built.length, 'accounts built')
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
          ...
          console.log('[ingestAllFiles] XLSX sheets:', wb.SheetNames)
          for (const sheetName of wb.SheetNames) {
            const tabType = xlsxSheetToTabType(sheetName)
            console.log('[ingestAllFiles]', sheetName, '->', tabType, tabType && newRaw[tabType] ? 'HAS BUCKET' : 'NO BUCKET')
            if (!tabType) continue
            if (!newRaw[tabType]) { newRaw[tabType] = [] }
            const ws = wb.Sheets[sheetName]
            const csvText = XLSX.utils.sheet_to_csv(ws)
            const sheetRecords = parseCSV(csvText)
            console.log('[ingestAllFiles]', sheetName, ': parsed', sheetRecords.length, 'records, first keys:', sheetRecords[0] ? Object.keys(sheetRecords[0]).join(', ') : 'NONE')
