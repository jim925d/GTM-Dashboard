...
export const DEFS = {
  ...
  'PRIORITY RANKING': 'Accounts ranked by composite priority score (0-100). Score factors: active deals, engagement recency, open needs, win probability, on-net presence, TMR size, and risk level.',
  ...
  // ── Overview stats ──
  'TOTAL TMR': 'Total Monthly Recurring revenue from the Total BRR field in customers.csv. Represents the customer\'s current contracted monthly spend.',
  ...
  'WEIGHTED PIPELINE': 'SUM(Deal MRR × Stage Win Probability × 12). Probability-adjusted TMR from current pipeline.',
  'ACTIVE DEALS': 'Current non-closed deals with positive MRR in the pipeline. Premier channel only.',
  'WEIGHTED PIPELINE SUMMARY': 'Probability-adjusted pipeline TMR. Each deal\'s MRR is multiplied by its stage win probability and annualized (×12).',
  ...
  'MONTHLY CHURN TREND (TMR)': 'Monthly churn TMR from deals with negative MRR. Shows revenue contraction trend over time. Premier channel only.',
  'CHURN BY PRODUCT/TYPE': 'Churn TMR broken down by product group. Identifies which products are most vulnerable to revenue loss.',
  'AT-RISK TMR': 'Total TMR from accounts with a risk score ≥ 30. These accounts show warning signs of potential churn.',
  'AVG CHURN DEAL': 'Average TMR per churn deal. Calculated as total churn TMR ÷ number of churn deals.',
  'CHURN RATE': 'Percentage of accounts classified as at-risk (risk score ≥ 30) out of total accounts.',
  'PRODUCT BY PIPELINE (TMR)': 'Active pipeline TMR broken down by product group. Shows which products have the most open pipeline value.',
  'BOOKINGS BY PRODUCT (TMR)': 'Closed-won bookings TMR broken down by product group. Shows which products are generating the most revenue.',
  ...
  'RAW PIPELINE TMR': 'Total raw TMR in active pipeline, positive MRR only (MRR × 12). Premier channel.',
  'WEIGHTED PIPELINE TMR': 'SUM(Deal MRR × Stage Win Probability × 12). The probability-adjusted view of the pipeline.',
  ...
  'BOOK OF BUSINESS': 'Total TMR across all accounts owned by this seller.',
  ...
  'AVG DEAL SIZE': 'Average TMR per closed-won deal. Total bookings TMR ÷ number of closed deals.',
  'PIPELINE GENERATED': 'Total TMR in active pipeline deals. Represents future revenue opportunity.',
